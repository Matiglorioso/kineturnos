...
/** Vista previa decorativa del panel — refuerza la propuesta de valor en el login desktop. */
export function AuthPanelPreview() {
  return (
    ...
    >
      <div className="absolute -right-3 top-6 h-full w-full rotate-3 rounded-2xl bg-white/10 ring-1 ring-white/20" />
      <div className="relative overflow-hidden rounded-2xl bg-white/95 p-4 text-slate-900 shadow-2xl shadow-teal-950/30 ring-1 ring-white/40 backdrop-blur-sm">
...
function PreviewAppointment({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-slate-100 bg-white px-2.5 py-2">
      ...
      <span className={`shrink-0 rounded-md px-1.5 py-0.5 text-[10px] font-medium ${statusClass}`}>
        {status}
      </span>
    </div>
  );
