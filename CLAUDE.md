...
**Auth y permisos:** `src/middleware.ts` protege las páginas con el JWT (sin Prisma: corre en edge). En la API, `requireApiSession` / `requireApiPermission` (`src/lib/auth/require-session.ts`) revalidan el usuario contra la tabla `usuarios` en cada llamada (usuario desactivado → 401; el rol sale de la base, no del JWT). Matriz rol × permiso en `src/lib/auth/permissions.ts`. El rol `profesional` queda acotado a sus propios turnos (`getOwnProfessionalId`). `safeCallbackUrl` evita redirects externos después del login. Hay rate limit de login por email e IP persistido en `login_intentos`.
